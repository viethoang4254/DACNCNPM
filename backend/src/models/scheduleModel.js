...
const scheduleSelectSql = `SELECT ts.id, ts.tour_id, t.ten_tour, t.so_nguoi_toi_da,
	   ...
	   ts.is_on_sale, ts.discount_percent, ts.auto_sale_applied, ts.auto_sale_opt_out
...
export const applySaleToScheduleById = async (id, discountPercent = 20) => {
	const [result] = await pool.execute(
		"UPDATE tour_schedules SET is_on_sale = TRUE, discount_percent = ?, auto_sale_applied = FALSE, auto_sale_opt_out = FALSE WHERE id = ?",
...
export const removeSaleFromScheduleById = async (id) => {
	const [result] = await pool.execute(
		"UPDATE tour_schedules SET is_on_sale = FALSE, discount_percent = 0, auto_sale_applied = FALSE, auto_sale_opt_out = TRUE WHERE id = ?",
		[id],
	);
